...
//It simulates a cellular automaton model (see Sankaran et. al. 2017, Ecological Indicators) on a landscape with a gradient of driver. 
...
const int N = 128;  // Size of the matrix having same driver value (driver vlaue changes after every N cells)
const int p_range = 181 ;  // No. of driver values in the transect from left to right 
const int T = 2000 ;  // No. of time steps for the simulations

int Nx=N, Ny=N*p_range+4; //Nx is the breadth and Ny is the length of transect 2 columns of left and 2 columns on right as assigned to implement fixed boundary on two sides
...
int main(){
	...
	p[0]=0.2400;   // driver value(p) at the left of thr matrix (landcape)
	for(x=1;x<p_range;++x) {
		p[x]= p[x-1] + 0.0001 ; //Driver value (p) increases along the length of the matrix after every N cells
	}
	...
	create_random_matrix(A,0.5);  
				...
				if (j>1 && j<Ny-2){   // We dont consider the first two and last two columns for fixed boundary
					...
					if (A[i*Ny+j]==1){     //if the site is occupied
						...
						if (A[in*Ny +jn]==0) {                     //if neighbor is empty
							if (test < p[x]) 
								A[in*Ny+jn]=1;                 //regular contact process
							else A[i*Ny+j]=0;
						} 
	 ...
	 // Now fixing the left boundary based on the density of the left matrix
			...
			outdata.open("density.dat",ios::app);
			snapshots.open("snapshots.dat",ios::app);
			...
			for (x=0;x<p_range;++x){
				...
				for (i=0;i<N; ++i){     //since N=Nx
					for (j=x*N+2;j<x*N+N+2;++j){      //we are calculating average of NxN matrix for one p-value
						mean += A[i*Ny+j];
						snapshots<<A[i*Ny+j]<<endl;   // Output file will have NxN cells in on column and then next NxN cells after that and so on till highest value of p.
